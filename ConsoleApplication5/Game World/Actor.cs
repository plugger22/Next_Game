...
namespace Next_Game
{
    ...
    public class Actor
    {
        ...
        /// <returns></returns>
        public string GetTraitEffectText(SkillType skill, SkillAge age = SkillAge.Fifteen)
        {
            string text = null;
            string plus = null;
        ...
        /// <summary>
        /// Change Relationship status -> if true, Relationship levels and history are shown to player
        /// </summary>
        public void SetRelationshipStatus(bool status)
        {
            RelKnown = status;
            Game.logTurn?.Write($"[Notification] {Title} {Name}, ActID {ActID} -> RelKnown changed to {status}");
        }
        ...
        internal string AddCondition(Condition condition)
        {
            ...
            if (condition != null)
            {
                if (condition.Skill != SkillType.None)
                {
                    ...
                    Record record = new Record(conditionText, ActID, LocID, CurrentActorEvent.Condition);
        ...
        public string ChangeResources(int amount, EventCalc calc)
        {
            ...
            if (String.IsNullOrEmpty(messageText) == false)
            {
                ...
                if (ActID == 1)
                { Game.world.SetPlayerRecord(new Record(messageText, ActID, LocID, CurrentActorEvent.Resource)); }
                else if (ActID > 1)
                { Game.world.SetCurrentRecord(new Record(messageText, ActID, LocID, CurrentActorEvent.Resource)); }
            }
    ...
    //Special NPC's & Beasts (for hunting)
    ...
    public class Enemy : Actor
    {
        ...
        public bool AIControl { get; set; } //true if AI controlled (can be manual in Act Two), default true;
        //other
